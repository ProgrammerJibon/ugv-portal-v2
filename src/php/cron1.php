...
$runTime = 0;
$runTime = 1766150485;
$runEndTime = $runTime > 0 ? $runTime + (10 * 60) : time() + 1;

$result = "";

if( time() > $runEndTime ) {
    if(!file_exists(__DIR__ . "/result-end.txt")) {
        file_put_contents(__DIR__ . "/result-end.txt", "Cron job ended at " . date("Y-m-d H:i:s") . "\n");
    }    
}elseif (time() > $runTime) {
    ...
    $result .= "--------------------------\n\n";
    if(!file_exists(__DIR__ . "/cron-logs")){
        mkdir(__DIR__ . "/cron-logs", 0777, true);
    }
    file_put_contents(__DIR__ . "/cron-logs/result-" . date("y-m-d-H-i-s") .".txt", $result, FILE_APPEND);
} else {
    // update time 
